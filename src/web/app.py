...
class ResourceMonitor(threading.Thread):
    ...
    def __init__(self, execution_id, process):
        threading.Thread.__init__(self)
        self.execution_id = execution_id
        self.process = process  # 直接传入process而非sandbox对象
        self.running = True
        self.data = []
    
    def run(self):
        start_time = time.time()
        while self.running and self.process and self.process.is_running():
            try:
                # 获取资源使用情况
                ...
                # 记录数据并打印调试信息
                print(f"监控 - 内存: {memory_mb:.2f}MB, CPU: {cpu_percent:.2f}%")
...
def execute_code():
    ...
    # 启动资源监控线程
    monitor = ResourceMonitor(execution_id, current_process)
    monitor.start()
    ...
    try:
        ...
        # 停止资源监控
        monitor.stop()
        monitor.join(timeout=1.0)  # 等待监控线程结束
        ...
        if execution_id in resource_data:
            print(f"资源数据点数量: {len(resource_data[execution_id])}")
        else:
            print("没有收集到资源数据")
